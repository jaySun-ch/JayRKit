...
- (void)_setter_ : (_type_) object{ \
    [EventName willChangeValueForKey:@#_getter_];\
    objc_setAssociatedObject(EventName,_cmd,object,OBJC_ASSOCIATION_ ## _association_);\
    [EventName didChangeValueForKey:@#_getter_];\
}\
- (_type_)_getter_ {\
    return objc_getAssociatedObject(EventName,@selector(_setter_:));\
}
#endif

#ifndef JR_DYNAMIC_PROPERTY_CTYPE
#define JR_DYNAMIC_PROPERTY_CTYPE(_getter_, _setter_, _type_)\
- (void)_setter_ : (_type_)object{ \
    [EventName willChangeValueForKey:@#_getter_]; \
    NSValue *value = [NSValue value:&object withObjCType:@encode(_type_)]; \
    objc_setAssociatedObject(EventName,_cmd,value,OBJC_ASSOCIATION_RETAIN);\
    [EventName didChangeValueForKey:@#_getter_];\
}\
- (_type_)_getter_ { \
    _type_ cValue = { 0 }; \
    NSValue *value = objc_getAssociatedObject(EventName,@selector(_setter_:));\
